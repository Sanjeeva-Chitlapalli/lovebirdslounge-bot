  ...
  try {
    ...
    for (const r of rawReminders) {
      ...
      for (const { lineUserId, name, partnerName } of targets) {
        ...
        let textDt = String(r.scheduledAt);
        // Remove trailing timezone names generated by Gemini occasionally (e.g. "2026-02-24T08:00:00 Asia/Bangkok")
        textDt = textDt.split(' ')[0];

        const scheduledAt = new Date(textDt);
        if (isNaN(scheduledAt.getTime())) {
          console.warn(`[Extraction] Invalid scheduledAt "${textDt}" (original: "${r.scheduledAt}") — skipped`);
          continue;
        }
